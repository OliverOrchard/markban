...
{
    public static void Execute(string rootPath, string title, string lane, string? afterId, bool topPriority)
    {
        var validFolders = new[] { "Todo", "In Progress", "Testing", "Done", "Ideas", "Rejected" };
        var normalizedLane = validFolders.FirstOrDefault(f =>
            f.Equals(lane, StringComparison.OrdinalIgnoreCase) ||
            f.Replace(" ", "").Equals(lane.Replace(" ", ""), StringComparison.OrdinalIgnoreCase)) ?? "Todo";
        ...
        string newId = "";
        if (normalizedLane != "Ideas" && normalizedLane != "Rejected")
        {
    ...
    public static void ExecuteSubItem(string rootPath, string title, string parentId, string lane, string? afterId)
    {
